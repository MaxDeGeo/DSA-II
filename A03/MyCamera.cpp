...
{
//Normalizes the forward vector and moves the object that much depending on direction of movement.
...
void MyCamera::MoveSideways(float a_fDistance){
	//Normalizes the right vector and moves the object that much depending on direction of movement.
...
//ChangeYaw rotates the object about the y axis.
void MyCamera::ChangeYaw(float yAngle)
{
	quaternion yQuat = glm::angleAxis(yAngle, AXIS_Y); //Gets the quaternion to rotate.

	m_v3Forward = m_v3Target - m_v3Position; //Creates the forward vector of the current object.
	//glm::normalize(m_v3Forward); //Normalizes that vector.

	m_v3Orientation = yQuat * m_v3Forward; //Sets the orientation by multipling the vector by the quaternion.
	
	m_v3Target = m_v3Position + m_v3Orientation; //Sets the target to the new position plus the orientation.
}

//ChangePitch rorates the object about the x axis.
void MyCamera::ChangePitch(float xAngle)
{
	quaternion xQuat = glm::angleAxis(xAngle, glm::cross(m_v3Forward, AXIS_Y)); //Gets the quaternion to rotate.

	m_v3Forward = m_v3Target - m_v3Position; //Creates the forward vector of the current object.

	m_v3Orientation = xQuat * m_v3Forward; //Sets the orientation by mulitplying the vector by the quaternion.

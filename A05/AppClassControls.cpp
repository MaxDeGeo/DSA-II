...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	case sf::Keyboard::F:
		...
		break;
	case sf::Keyboard::PageUp: //Included in program
		++m_uOctantID;
		if (m_uOctantID >= m_pRoot->GetOctantCount())
			m_uOctantID = - 1;
		break;
	case sf::Keyboard::PageDown: //Included in program
		--m_uOctantID;
		if (m_uOctantID >= m_pRoot->GetOctantCount())
			m_uOctantID = - 1;
		break;
	case sf::Keyboard::Add: //Included in program
		...
		break;
	case sf::Keyboard::Subtract: //Included in program

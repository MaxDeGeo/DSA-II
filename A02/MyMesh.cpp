...
void MyMesh::GenerateSphere(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	float xCoor, yCoor, zCoor, xyCoor; // vertex position
	...
	for (uint x = 0; x <= a_nSubdivisions; ++x)
	{
		...
		xyCoor = a_fRadius * glm::cos(stackAngle);
		zCoor = a_fRadius * glm::sin(stackAngle);
		...
		for (int y = 0; y <= a_nSubdivisions; ++y)
		{
			...
			xCoor = xyCoor * glm::cos(sectorAngle);
			yCoor = xyCoor * glm::sin(sectorAngle);

			vertices.push_back(vector3(xCoor, yCoor, zCoor));
		}
	}
	...
	{
		point1 = x * (a_nSubdivisions + 1);     // beginning of current stack
		point2 = point1 + a_nSubdivisions + 1;      // beginning of next stack

		for (int y = 0; y < a_nSubdivisions; ++y, ++point1, ++point2)
		{
			// 2 triangles per sector excluding first and last stacks
			// point1 => point2 => point1+1
			if (x != 0)
				AddTri(vertices[point1], vertices[point2], vertices[point1 + 1]);

			// point1+1 => point2 => point2+1
			if (x != (a_nSubdivisions - 1))
				AddTri(vertices[point1 + 1], vertices[point2], vertices[point2 + 1]);
		}
	}
	// -------------------------------
	// Adding information about color

...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	//Sets the initial mins and maxs of the box in local space.
	cornerPoints[0] = m_v3MinL;
	cornerPoints[1] = vector3(m_v3MinL.x, m_v3MinL.y, m_v3MaxL.z);
	cornerPoints[2] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MaxL.z);
	cornerPoints[3] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z);
	cornerPoints[4] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z);
	cornerPoints[5] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MinL.z);
	cornerPoints[6] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MaxL.z);
	cornerPoints[7] = m_v3MaxL;

	//Makes those points in global space.
	for (int x = 0; x < 8; x++)
	{
		cornerPoints[x] = vector3(m_m4ToWorld * vector4(cornerPoints[x], 1.0f));
	}

	//Sets initial starting points for min and max.
	m_v3MaxG = cornerPoints[0];
	m_v3MinG = cornerPoints[0];

	//Finds the min and max of the ARBB.
	for (int x = 0; x < 8; x++)
	{
		if (m_v3MaxG.x < cornerPoints[x].x)
			m_v3MaxG.x = cornerPoints[x].x;
		else if (m_v3MinG.x > cornerPoints[x].x)
			m_v3MinG.x = cornerPoints[x].x;

		if (m_v3MaxG.y < cornerPoints[x].y)
			m_v3MaxG.y = cornerPoints[x].y;
		else if (m_v3MinG.y > cornerPoints[x].y)
			m_v3MinG.y = cornerPoints[x].y;

		if (m_v3MaxG.z < cornerPoints[x].z)
			m_v3MaxG.z = cornerPoints[x].z;
		else if (m_v3MinG.z > cornerPoints[x].z)
			m_v3MinG.z = cornerPoints[x].z;
	}
	//----------------------------------------
